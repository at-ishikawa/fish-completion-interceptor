...
function run_ghq_test
    __fci_plugin_run_test \
        --plugin_function=fci_plugin_ghq_fzf \
        $argv
end
...
function run_supported_command_test_cases
    ...
    run_ghq_test \
        ...
        --expected_fzf_option "$option --preview=gh repo view {1} --query=$__FCI_PLUGIN_GH_FZF_DEFAULT_ORG/fish $bind_option $FISH_COMPLETION_INTERCEPTOR_FZF_OPTIONS" \
        --mock_fzf_stdout "fish/fish fish shell" \
        --expected_stdout "fish/fish"
    run_ghq_test \
